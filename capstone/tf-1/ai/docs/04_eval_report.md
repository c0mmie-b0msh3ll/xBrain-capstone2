...
- Persistent audit storage is implemented for W11 as local append-only JSONL through `app/audit_store.py`, with metadata-only decision lineage, request/evidence hashes, mode/model/tool lineage, ticket lineage, guardrail flags, and a retention target of at least 90 days. This is documented as an implementation/handoff capability, not a change to the frozen W11 contracts at `ccbb47f`. Production hosting can replace the local path with CDO-approved durable storage such as S3 Object Lock or DynamoDB without changing `/v1/triage`.
